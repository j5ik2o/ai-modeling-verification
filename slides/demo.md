...
```
EV の充電セッションを想定した課金処理を 2つの独立した実装でそろえてください。
どちらもスタート時刻と単価を受け取り、途中でスナップショット課金を取ったり、最後に停止して確定金額を返せるようにしてほしいです。

現場では、短時間の無料枠や負の入力の拒否、現実的な課金上限など、ありそうなビジネスルールを守れることが期待されています。
将来追加しそうなケース（極端に長時間の利用、異常な数値の入力、停止後にさらに請求しようとする操作など）にも破綻しないよう、テストを通じて確認してください。
仕様はこれくらいの粒度しか共有できませんが、後から細かな条件を足しても破綻しない堅い設計とテストをお願いします。
```
