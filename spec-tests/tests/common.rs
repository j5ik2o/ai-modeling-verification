...
pub const STOP_CASES: &[StopCase] = &[
  StopCase {
    ...
    energy_milli:     2_400,
    rate_yen_per_kwh: 50,
    expected:         BillingResult { billed_energy_milli: 400, amount_yen: 20 },
  },
  StopCase {
    ...
    energy_milli:     1_000,
    rate_yen_per_kwh: 80,
    expected:         BillingResult { billed_energy_milli: 0, amount_yen: 0 },
  },
  StopCase {
  ...
  StopCase {
    ...
    energy_milli:     6_000,
    rate_yen_per_kwh: 40,
    expected:         BillingResult { billed_energy_milli: 4_500, amount_yen: 180 },
  },
  StopCase {
  ...
  StopCase {
    ...
    expected:         BillingResult { billed_energy_milli: 500_000, amount_yen: 1_000_000 },
  },
];
...
where
  ...
  E: std::fmt::Display, {
  let session = S::start(BASE_START_MS, 2_001)
    .unwrap_or_else(|err| panic!("{} start failed for over-limit scenario: {}", model_name, err));
  let snapshot = session.bill_snapshot(end_timestamp(10), 1_000_000);
  assert!(snapshot.is_err(), "{} should reject snapshot when amount exceeds limit", model_name);
  let stop_result = session.stop(end_timestamp(10), 1_000_000);
  assert!(stop_result.is_err(), "{} should reject stop when amount exceeds limit", model_name);
}
...
where
  ...
  assert!(snapshot.is_err(), "{} should reject snapshot when energy exceeds limit", model_name);
  let stop_result = session.stop(end_timestamp(10), over_limit);
  assert!(stop_result.is_err(), "{} should reject stop when energy exceeds limit", model_name);
}
...
where
  ...
  assert_eq!(result.amount_yen, case.expected.amount_yen, "{} amount mismatch for {}", model_name, case.name);
  result
}
...
where
  ...
  S::ClosedSession: ClosedBillingSession<Error = E>,
  E: std::fmt::Display, {
  let session = S::start(BASE_START_MS, rate).unwrap_or_else(|err| panic!("{} start failed: {}", model_name, err));
  ...
  for (minute, energy, expected) in snapshots {
    ...
    assert_eq!(result.amount_yen, expected.amount_yen, "{} amount mismatch at minute {}", model_name, minute);
...
where
  ...
  S::ClosedSession: ClosedBillingSession<Error = E>,
  E: std::fmt::Display, {
  let session = S::start(BASE_START_MS, 50).unwrap_or_else(|err| panic!("{} start failed: {}", model_name, err));
  let (_, closed) =
    session.stop(end_timestamp(6), 2_400).unwrap_or_else(|err| panic!("{} stop failed: {}", model_name, err));

  let result = closed.bill_after_stop(end_timestamp(7), 2_800);
  assert!(result.is_err(), "{} should reject billing after stop", model_name);
}
...
where
  ...
  S::ClosedSession: ClosedBillingSession<Error = E>,
  E: std::fmt::Display, {
  let session = S::start(BASE_START_MS, 45).unwrap_or_else(|err| panic!("{} start failed: {}", model_name, err));
  let snapshot = session.bill_snapshot(end_timestamp(4), -500);
  assert!(snapshot.is_err(), "{} should reject negative energy snapshot", model_name);

  let stop_result = session.stop(end_timestamp(6), -500);
  assert!(stop_result.is_err(), "{} should reject negative energy stop", model_name);
}
...
where
  ...
  S::ClosedSession: ClosedBillingSession<Error = E>,
  E: std::fmt::Display, {
  let session = S::start(BASE_START_MS, 60).unwrap_or_else(|err| panic!("{} start failed: {}", model_name, err));
  let snapshot = session.bill_snapshot(BASE_START_MS - MINUTE_MS, 1_000);
  assert!(snapshot.is_err(), "{} should reject reversed timeline snapshot", model_name);

  let stop_result = session.stop(BASE_START_MS - MINUTE_MS, 1_000);
  assert!(stop_result.is_err(), "{} should reject reversed timeline stop", model_name);
}
...
where
  ...
  let case = StopCase {
    ...
    energy_milli:     3_600,
    rate_yen_per_kwh: 70,
    expected:         BillingResult { billed_energy_milli: 1_600, amount_yen: 112 },
  };
